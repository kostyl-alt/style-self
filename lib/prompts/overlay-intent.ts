...
- brand-learn         : ブランド/デザイナー/美学(アバンギャルド/ミニマル/デコンストラクション等)の学習 + 自分の世界観との相性確認・推薦
...
1. ブランド名 / デザイナー名 / 美学キーワード(アバンギャルド・ミニマル・デコンストラクション・コンセプチュアル等)+ 学習意図 → brand-learn
2. 音楽 / 映画 / アート / カルチャー文脈の学習 → culture
3. 抽象テーマ(「静けさ」「余白」等)起点でコーデのアイデア要求 → inspiration
4. 服選び全般の悩み(ブランド名なし)→ style-consult

★ 重要 (A-6b): 「自分の世界観に合うブランドを知りたい」「○○ブランドは自分に合う?」は brand-learn(マッチング型)。ブランドが特定されておらず雰囲気だけ指定された場合(「黒系のブランドを教えて」)も brand-learn に分類する。
