...
[[SECTION:signals]] 機械可読の事実タグ（保存用・画面には出ない）
...
[[SECTION:signals]] 機械可読の保存用タグ。下の5行を必ずこの形（ラベル: 値, 値）で、カンマ区切りの日本語タグだけ書く（文章・理由・記号・英語スラッグは書かない）。値が無ければラベルだけ書いて空でよい。
色: 黒, グレー
シルエット: ワイド, Iライン
ジャンル候補: 90sストリート, 韓国ノームコア
年代: 1990s
ムード: 色数少なめ, 装飾少なめ
⚠️ signals の値は上の表示セクション（reference/materials/visible_facts 等）で挙げた分析と整合させる（表示で「90sストリート/グランジ」と言ったら signals のジャンル候補も同じに・ズレさせない）。ジャンル/年代は候補（断定でなく）。英語スラッグは書かない。
