...
#### 申し送り(M3-5 完了時記録・次回確実に参照)

- **admin アカウントのログイン認証情報を整備する**
  - 検証で複数アカウントを行き来する際、確実に admin に
    入れる状態にしておくこと。M3-5 検証時に「admin で
    削除した」と思った操作が実は被害者本人のセッションで
    削除していた、というログイン取り違えで重大バグの誤認が
    発生した(コード層・RLS層は健全と SQL 直接確認で実証済み)
- **admin での実機削除テスト(他人id DELETE→404)の後追い**
  - 認証情報整備後に念のため後追い実施可能。
  - 必須ではない: 本人保証はコード層(`.eq author_user_id` +
    body 不受)+ RLS 層(`auth.uid() = author_user_id` for ALL・
    SQL で `rls_enabled=true` + ポリシー実体を確認済み)の
    二重で保証されている
