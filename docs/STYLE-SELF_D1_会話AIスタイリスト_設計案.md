...
>    に正式統合済**(2026-05-20 オーナー 5 判断確定後・本コミット内で本体更新)。
>    本ファイルは **議論材料 / 会話 AI 化経緯の記録**として残す(削除しない)。
>
> 統合された 5 判断:
>   1) ステップ組込 = 案 X(P1-C-1.5 を P1-C-2 の前に挟む)
>   2) 段階 B モデル = まず Haiku 4.5 検証 → 不足なら Sonnet 4.6 昇格
>   3) MVP 初手 = MVP-1(良い例 1-2:診断 / クローゼットから)
>   4) コスト許容 = MVP 検証は進める・将来の利用回数制限は本体 7.4 に「将来対応」として明記
>   5) 本体改訂タイミング = 合意直後(=本コミット)
