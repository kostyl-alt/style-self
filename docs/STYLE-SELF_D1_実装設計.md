...
## 0. D-1 の本質(★ 案A 確定・タブなし完全チャット型・最新改訂版)
...
オーナー方針の純化:
  「ChatGPT の服版 = チャットがアプリそのもの」
  「下部ナビすら不要。アプリを開いたらチャット画面しかない」
...
転換 2: 「対話中心(オーバーレイ型)」へ転換
        ...
        ↓ 2026-05-20 オーナー実機 FB(午前)
        「FAB + タブ併存は『普通アプリに AI を足した形』で
         ChatGPT の服版になっていない。チャットそのものをメイン画面に」
転換 3: 「チャット主役型(下部ナビ 3 タブ)」へ転換
        メイン画面 = /ai・FAB 廃止・下部ナビ AI/保存/自分(P1-A `c77a54c` / P1-B `79a76be`)
        詳細経緯: docs/STYLE-SELF_D1_チャット主役型_設計案.md(本体統合済)
...
```
D-1 = 「アプリを開いたらチャット画面しかない。
        既存 18 機能は自然文 or 右上メニューから全到達する」を実現する。

メイン画面 = /ai(チャット画面のみ・下部ナビなし)
  上部:    STYLE-SELF AI + 世界観カード(小)+ [≡] 右上メニュー
  中央:    チャット履歴(吹き出し列・スクロール)
  下部:    大入力欄 + 入力欄近接ボタン
           [📎写真追加] [🔗商品 URL] [👕クローゼット参照] [🎨MB追加]
  初回:    「今日は何を相談しますか?」+ 提案チップ 5 種
           ・この服に何合わせれば
           ・世界観に合うコーデ作って
           ・買うか迷ってる服を相談
           ・ムードボードから作る
...
右上メニュー [≡]:
  ・あなたの世界観 / クローゼット / 保存 / 身体・好み・避けたい / 履歴 / 投稿 / 設定 / ログアウト
  → 前面に主張しない・「いざという時の手段」として残す

チャットコマンド(D1-1 intent + D1-2a navigate-map を★そのまま転用・新規実装ゼロ):
  「保存見せて」     → /saved
  「俺の世界観見せて」→ /self?tab=diagnosis
  「体型変更したい」 → /self?tab=body
  「クローゼットに追加」「服を登録」→ /outfit?tab=closet
  「投稿作りたい」   → /self/new-post
  「再診断したい」   → /onboarding
  「履歴見せて」     → /self?tab=history
  「好み編集」       → /self?tab=worldview

AI 返答は 文章 + カード形式(対話完結 8 機能):
  結論 / 合う理由 / 合わない可能性 / 代替案 / コーデ案 /
  次アクション [💾 保存・🛍 商品を探す・🔄 別案](Phase 1)
  + [👗 着せてみる・📌 MB へ](Phase 2/3 で追加)

= ChatGPT の服版 × リアル試着 × 世界観一貫
```

### ★ 変えないもの(案A・タブなし完全チャット型でも完全保持)

```
既存 18 機能・全 API・全 DB・M2-3 / M4-2 / M5 / M3 プライバシー設計
★ /u/[id] /p/[id] 公開ページ URL 構造(SNS シェア互換・絶対不変)
  → app/(public)/ ルートグループ独立・BottomNav 撤去で影響ゼロ(構造的に独立)
M2-3 worldview_tags 非露出原則
③ プライバシー専章(本ドキュメント 6 章)・③ コスト管理(7 章)・Phase 2 後 GO ゲート

= 変わるのは「ユーザーがどの画面で機能に到達するか」の導線だけ。
  機能・API・データ・プライバシー設計は 4 段階の純化を通して 一切変えない。
```
...
判断8: ★ オーナー実機 FB を受けた案A確定(2026-05-20 午後・タブなし完全チャット化)
       ...
       チャットコマンド = D1-1 intent ルーター + D1-2a navigate-map を ★そのまま転用
                         (新規実装ゼロで 9 件の専用 UI 引き継ぎが成立 ─ 比較設計案の結論)
...
判断10: ★ 8 パターン廃止スコープ(doc7「①知る再定義」確定・2026-05-22・詳細: docs/STYLE-SELF_診断システム_再設計.md)
       1) 8 パターン体制廃止 = AI が毎回ユーザー固有世界観を構築(analyze-v2)
       2) Phase A(analyze-v2) ✅ 完了・1.5b 完成形は既に analyze-v2 schema
          (worldviewName / worldview_keywords / coreIdentity / idealSelf)で動作(4.7 参照)→ 既存達成と完全整合
       3) 過去診断データ = 案 a(過去は 8 パターン形式温存・再診断で新形式上書き)
       4) Phase C(WORLDVIEW_PATTERNS / patternId 等の legacy 削除)= MVP-1c 後の別 Sprint(衝突回避・M5 刻む作法)
       5) T2(8 パターン legacy 監査・別 doc)= 本判断確定後すぐに投入予定
```
...
### 4.3 案A・タブなし完全チャット型 UI 配置(★判断 8: 下部ナビ廃止)

| 候補 | 評価 | 採用 |
|---|---|---|
| (旧A) FAB + モーダル(対話中心改訂)| BottomNav と共存・対話画面はモーダル内 | **廃止**(判断 7 で否決)|
| (旧B) 全画面オーバーレイ | ChatGPT 型・対話に集中 | (旧 D-2 候補)|
| (旧C) チャット主役型 + 下部ナビ 3 タブ | チャットがメイン + AI/保存/自分 タブ | **廃止**(判断 8 で否決・「タブある普通アプリ」感)|
| **(新) 案A タブなし完全チャット型** | アプリ = /ai のチャットのみ・下部ナビなし・右上メニュー + チャットコマンド | ★ **MVP 採用**(判断 8)|

#### メインチャット画面 `/ai` の構造(P1-C で実装・案A)

```
新規ルート: /ai(判断 7-1)
起動導線: app/page.tsx redirect /home → /ai(判断 7-3・P1-A 完了)
middleware.ts: appRoutes に "/ai" 追加(P1-A 完了)
下部ナビ: 廃止(判断 8・P1-C で BottomNav 撤去 = メインチャット画面では非表示)
...
画面構成:
  ...
  │  「今日は何を相談しますか?」                 │
  │  ┌──────────────┐ ┌──────────────┐         │
  │  │💡 この服に何 │ │💡 世界観に合う│         │ ← 提案チップ 5 種
  │  │  合わせれば  │ │  コーデ作って │         │   (補助・初回 onboarding 的)
  │  └──────────────┘ └──────────────┘         │
  │  ┌──────────────┐ ┌──────────────┐         │
  │  │💬 買うか迷ってる │ │🎨 ムードボード │         │
  │  │  服を相談    │ │  から作る   │         │
  ...
  │                                               │
  ├─────────────────────────────────────────────┤
  │ [📎写真] [🔗商品URL] [👕クローゼット] [🎨MB] │ ← 入力欄近接ボタン
  │ ┌────────────────────────────────────┐ [送]│
  │ │ 自然文で書いてください…              │     │
  │ └────────────────────────────────────┘     │
  └─────────────────────────────────────────────┘
※ 下部ナビなし(★判断 8 = BottomNav 撤去 = 案A の核)
```

#### 右上メニュー [≡] 設計(★判断 8・補助機能の集約点・前面に主張しない)

```
[≡] タップで右からスライド展開:
┌──────────────────────┐
│ あなたの世界観        │ → /self?tab=diagnosis(navigate-map 経由)
│ クローゼット          │ → /outfit?tab=closet
│ 保存                  │ → /saved
│ 身体・好み・避けたい  │ → /self?tab=body / =worldview
│ 履歴                  │ → /self?tab=history
│ 投稿                  │ → /self?tab=posts(M3 自分の投稿管理・判断 7-5)
│ ──────────────       │
│ 設定                  │ → 将来
│ ログアウト            │ → /auth/signout(既存)
└──────────────────────┘

毎日使う機能(コーデ提案・相談・マッチ)はチャット完結のため、
右上メニューは実質「設定とプロフィール管理の入口」として残す。
```

#### チャットコマンド設計(★判断 8・新規実装ゼロで成立)

D1-1 `/api/overlay/intent`(意図ルーター)+ D1-2a `lib/overlay/navigate-map.ts` を ★ **そのまま転用**:

| ユーザー発話例 | intent | 結果 |
|---|---|---|
| 「保存見せて」「保存リスト」| `saved` | 「保存リストを開きますね →」+ /saved |
| 「俺の世界観見せて」 | `worldview-profile` | + /self?tab=diagnosis(タブ命名トリック吸収済)|
| 「体型変更したい」「身長変えたい」| `body-edit` | + /self?tab=body |
| 「クローゼットに追加」「服を登録」| `closet` | + /outfit?tab=closet |
| 「投稿作りたい」 | `create-post` | + /self/new-post |
| 「自分の投稿見せて」 | `my-posts` | + /self?tab=posts |
| 「再診断したい」 | `diagnose` | + /onboarding |
| 「履歴見せて」 | `history` | + /self?tab=history |
| 「好みを編集したい」 | `preference-edit` | + /self?tab=worldview(タブ命名トリック吸収済)|

→ ★ navigate-map の **9 entry をそのまま転用** で全到達可能。新規 intent / 新規 navigate-map エントリ追加は不要。
...
### 4.4 既存機能を「壊さない」境界線(明文化・案A・タブなし完全チャット型でも完全保持)
...
#### ★ 案A 既存資産 到達手段マップ(★ 迷子ゼロ確認・判断 8 で正式反映)

| 既存資産 | 案A での到達手段 |
|---|---|
| **対話完結 8 機能** | チャット内完結(coordinate / style-consult / inspiration / match-users / match-posts / culture / brand-learn / virtual-coordinate 概要)|
| **専用 UI 引き継ぎ 10 機能** | チャットコマンド or 提案チップ 5 → navigate-map(D1-2a 9 entry)経由で各画面 |
| **M3 投稿作成** `/self/new-post` | チャットコマンド「投稿作りたい」or 右上メニュー「投稿」|
| **M3 自分の投稿管理** `/self?tab=posts` | 右上メニュー「投稿」or チャット「自分の投稿」(判断 7-5 = 自分タブ内維持)|
| **★ M2-3 公開プロフ `/u/[userId]`** | ★ **絶対不変**(SNS シェア URL・public ルートグループ独立・BottomNav 撤去で影響ゼロ)|
| **★ M3-4 投稿個別公開 `/p/[postId]`** | ★ **絶対不変**(同上)|
| **保存** `/saved` | 右上メニュー「保存」or チャット「保存見せて」|
| **クローゼット** `/outfit?tab=closet` | 右上メニュー「クローゼット」or 入力欄「👕クローゼット参照」ボタン or チャット「クローゼット見せて」|
| **身体情報編集** `/self?tab=body` | 右上メニュー or チャット「体型変更したい」|
| **好み編集** `/self?tab=worldview` | 右上メニュー or チャット「好み編集」(タブ命名トリック吸収済)|
| **AI 履歴** `/self?tab=history` | 右上メニュー or チャット「履歴見せて」|
| **再診断** `/onboarding` | チャット「再診断したい」|
...
| **/home(旧ホーム)** | 案A での到達手段なし(機能は他で全 cover・URL 直アクセスのみ生存)|

→ ★ **到達できない機能ゼロ**。/home は機能セットの集約画面で、内容は全て他で cover。

#### 公開ページ /u/[id] /p/[id] の絶対不変構造(★ 案A 採用でも変わらない)

```
app/(public)/u/[userId]/page.tsx
app/(public)/p/[postId]/page.tsx
  ↑ public ルートグループ = (app) と独立 = BottomNav 元から非表示
  ↑ middleware 認証ガード対象外(anon SSR)= 未ログインでもアクセス可
  ↑ M2-3 SELECT 列絞り / M4-2 public_users view / M3-4 fallback HTTP 200 で保護済
  ↑ 外部 SNS シェア URL として固定・絶対変更不可

= 案A で下部ナビを撤去しても、本ルート群は構造的に独立しているため影響ゼロ。
```
...
**★ analyze-v2 schema 前提(P1-C-1.5a 以降・判断 10 参照)**: `diagnose` 経路の `contextData` は `analyze-v2`(doc7 Phase A)出力スキーマで動作(`worldviewName` / `worldview_keywords` / `coreIdentity` / `idealSelf` を `worldview_profiles.result` jsonb から列絞り SELECT・[app/api/ai/stylist-chat/route.ts:204-206](../app/api/ai/stylist-chat/route.ts#L204-L206))。
8 パターン legacy(`WORLDVIEW_PATTERNS` / `patternId`)非依存・削除は MVP-1c 後の Phase C 予定。
...
| 項目 | 内容 |
|---|---|
| 上部 | STYLE-SELF AI 見出し + 世界観カード(小)で `worldviewName` を常時表示(例:「黒い美術館の住人 — 90 年代の静寂を纏う」)|
| 入力欄近接ボタン | [📎写真追加 / 🔗商品 URL / 👕クローゼット参照 / 🎨MB追加] の 4 ボタン(各 Phase 2 / 3 で本実装・P1-D は骨格のみ)|
| 初回 | 履歴空時に「今日は何を相談しますか?」+ 提案チップ **5 種**(案A 版・★ チップ数は判断 8 で 6→5 に整理)|
| チップ | ①💡 この服に何合わせれば / ②💡 世界観に合うコーデ作って / ③💬 買うか迷ってる服を相談 / ④🎨 ムードボードから作る / ⑤👗 自分に着せてみる |
| ★コスト最小化 | チップ選択時は **意図判定スキップ**(intent 直接付与で Haiku 1 回節約)|
| 完了条件 | チップタップで該当機能が直接トリガ + 世界観カードが常時可視 + 入力欄近接ボタンが配置済 |
...
| ★ 案A 到達マップ点検 | 4.4 の到達マップ通り 18 機能 + M3 投稿 + 公開 /u /p + 保存 + 自分 が **チャットコマンド + 右上メニュー + 入力欄近接ボタン** から全到達可能(★ 迷子ゼロ)|
| 退行点検 | 旧画面(/home /discover /outfit /saved /self が URL アクセスで従来通り)/ 公開ページ `/u/[id]` `/p/[id]` 通常動線無変更 / M3 投稿フロー無変更 / M2-3 / M4-2 / M5 既存 API 動作不変 |
| 知見 docs 追記 | 本ドキュメントセクション 13(M5 同型の知見小節)に P1 実装で得た知見を追記 |
| 完了条件 | 漏洩ゼロ + 到達マップ全項目 OK + 退行ゼロ + docs 追記済 + コスト実測ログ(¥3.7/月想定範囲内)|
...
docs/STYLE-SELF_ビジョン統合マップ.md(最上位)
  ├ STYLE-SELF_診断システム_再設計.md(①知る 思想・Phase A ✅ / B 🟡 / C ❌・★ 判断 10 参照)
...
> ★ ①知る再定義 進捗(2026-05-22・判断 10 参照):
> Phase A ✅(`analyze-v2` 並行実装)/ Phase B 🟡(`stylist-chat` は `analyze-v2` schema 使用済・`lib/knowledge/worldview-concepts.ts:99-100` 既知ギャップ残)/ Phase C ❌(MVP-1c 完了後の別 Sprint で投入予定)
