...
> ★ **ギャップ D(画像 URL / MB 入力)本実装の MB 側**(Sprint B-2 で配置確定): A-5 P1-D で 4 ボタン骨格完了(`fcbe065`)→ Sprint C で MB ボタン本実装(`moodboards` テーブル + Supabase Storage + RLS + チャット投入連鎖)。横断 TODO 13.6 MB 側を Sprint B-2 で配置確定済。
...
> ★ **ギャップ D(画像 URL / 写真投入)本実装の写真側**(Sprint B-2 で配置確定): A-5 P1-D で写真ボタン骨格完了(`fcbe065`)→ Sprint E で写真アップロード本実装(顔写真は ③ プライバシー専章準拠 / EXIF 除去 既存パイプライン流用 / `(public)` 公開ルートは列絞り SELECT)。横断 TODO 13.6 写真側を Sprint B-2 で配置確定済。

### 7.1 ★ E-0 リアル試着 Phase 1-2 体型モデル可視化(ギャップ A・Sprint B-2 で統合済)
- **入力**: M5 既存 `body_profile`(身長 / 体重 / ウエスト / 肩幅 / 股下)を活かす(MVP 優先 8 項目「リアル試着用 身体情報設計」既達成)
- **可視化**: 自分に近い体型モデル選択(2D / 3D 簡易表現・技術選定は E-1 と連動)
- **着せる**: 体型モデルにコーデを着せる(ビジョン `df36d82` リアル試着 Phase 1-2「自分に近い体型モデル」+「世界観コーデを着せる」)
- **規模**: +200-400 行想定(モデル UI + 体型補間・技術選定で振れ)
- **着手タイミング**: Phase 2 後ゲート(Sprint D)GO 後・Sprint E 先頭
- **横断 TODO**: 13.4(✅ Sprint B-2 で統合済)・MVP 優先 8 項目延長

### 7.2 E-1 試着技術選定 + プライバシー設計(本体 6 章 ③ 専章準拠)
### 7.3 E-2 簡易試着 MVP 実装
### 7.4 E-3 試着 → 保存 → 共有
### 7.5 E-4 完成・退行点検
...
### 8.3 F-3 試着 → 購入連鎖
### 8.4 ★ F-5 髪型 / メイク / カラコン提案(ギャップ B・Sprint B-2 で統合済・案 A 採用 = Sprint H 新設しない)
- **対象**: 似合う髪型 / 服に合う髪型 / 世界観に合う髪型 / メイクの方向性 / カラコンの方向性 / 顔まわりの印象調整(ビジョン `df36d82`「将来的に」明示)
- **位置づけ**: ★ MVP 後回し領域(ビジョン明示)・Phase 4 領域として自然延長
- **判断**: Sprint H 新設しない(改訂最小 + 章番号維持 + ビジョン Phase 4 整合・Sprint B-2 設計案 `5e879c7` §7 参照)
- **規模**: +150-300 行想定(prompt 拡張 + 結果カード追加・実装時に再計測)
- **着手タイミング**: F-1〜F-3 完了後・F-4 最終形達成より前(Phase 4 完成の構成要素として組込)
- **横断 TODO**: 13.5(✅ Sprint B-2 で統合済)

### 8.5 F-4 ★ Phase 4 完成・**最終形達成**(ChatGPT の服版 × リアル試着 × 世界観一貫)
...
1. ✅ **Sprint B-1 Phase 2 前ゲート評価**(完遂・`65973c5`・Phase 2 着手 GO 確定 + 4 項目評価 + 案 A 推奨)
2. ✅ **Sprint B-2 ビジョンマップ統合**(完遂・設計案 `5e879c7` + ロードマップ改訂 本 commit・doc3-6 配置確定 + ギャップ A/B/D Sprint 配置確定 + 横断 TODO 13.4-13.7 完了)
3. **★ Sprint B-3 コスト管理運用化**(★ **直近次工程**・案 P1 月 N 回上限実装・`users.chat_count_month` 追加 + stylist-chat 上限判定 +20-30 行)
4. **Sprint C ムードボード**(Phase 2 本実装・ギャップ D MB 本実装含む)
5. **拡張領域順次**(A-6c-e / A-7 / A-8 を必要性で順次着手)
...
- ✅ **Sprint B-2 で統合済**(本 commit / 設計案 `5e879c7` §4.2)→ [§7.1 E-0](#71--e-0-リアル試着-phase-1-2-体型モデル可視化ギャップ-asprint-b-2-で統合済) として配置確定
...
- ✅ **Sprint B-2 で統合済**(本 commit / 設計案 `5e879c7` §7 案 A 採用)→ [§8.4 F-5](#84--f-5-髪型--メイク--カラコン提案ギャップ-bsprint-b-2-で統合済案-a-採用--sprint-h-新設しない) として配置確定(Sprint H 新設しない)

### 13.6 画像 URL / MB 入力 本実装(整合性点検 `ddb86f7` ギャップ D)
- **現状**: A-5 P1-D で 4 ボタン骨格完了(`fcbe065`)・本実装は Sprint C(MB)/ Sprint E(写真)
- **改訂案**: A-5 完了後の各 Sprint で本実装(MB ボタン → Sprint C / 写真ボタン → Sprint E)
- **緊急性**: 中(A-5 骨格完了 → 本実装は各 Sprint で順次)
- ✅ **Sprint B-2 で統合済**(本 commit / 設計案 `5e879c7` §4.2)→ [§5 Sprint C 冒頭注記](#5-sprint-c-phase-2-ムードボード5-6-セッション) + [§7 Sprint E 冒頭注記](#7-sprint-e-phase-3-簡易試着5-8-セッションゲート通過時のみ) として明示

### 13.7 Sprint B-2 でまとめて統合(13.4-13.6 + B-2 統合工程)
...
- ✅ **Sprint B-2 で実行完遂**(設計案 `5e879c7` + 本 commit ロードマップ改訂)→ §7 E-0 + §8.4 F-5 + §5/§7 ギャップ D 注記 + 本 §13 完了マークで全完了
