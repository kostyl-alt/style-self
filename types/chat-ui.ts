...
export type MessageContent =
  ...
  | { kind: "photos-sent";   photoDataUrls: string[]; storagePaths?: string[]; caption?: string; factsSummary?: string }  // ★ 📷構造の送信バブル: 送った写真を通常サイズで表示。photoDataUrls=ライブ表示の base64・storagePaths=Supabase Storage 永続キー(署名URLで解決)。factsSummary=手持ち服コーデ相談の追撃会話用に vision facts を短いテキストで保持(黒・ワイド等・base64でなく軽い・buildStylistHistory が積む。closet 経路でのみ付与)
  ...
  | { kind: "style-match"; photos: { index: number; vision: MoodboardItemVision }[]; signals: MoodboardSignals; photoDataUrls?: string[]; storagePaths?: string[]; keywords?: StyleMatchKeywords; keywordsLoading?: boolean; keywordsError?: boolean }  // ★ Style Match Result(理想写真→「買える言葉」)。解析は photos-structure 流用。①写真一覧②抽出タグ(signals決定的)＋④アプリ別検索ワード(keywords=LLM整形・第2段)＋⑤外部検索ボタン。photoDataUrls=ライブ base64・storagePaths=Storage 永続キー(第2段)。③買う条件/DB保存は第3段(現状 ephemeral)
