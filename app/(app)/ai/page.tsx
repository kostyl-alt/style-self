...
// P1-C-1.5a / 1.5b-i / MVP-1c / A-6: 段階B 対象 intent(A-6 は {diagnose, closet, coordinate, style-consult})
// ★ ここに無い intent は従来通り intent-result(NavigateConfirm 等)で表示する。
// ★ API 側 `app/api/ai/stylist-chat/route.ts` の同名 Set と完全一致させる(両側同期)
// ★ L4-A 切替検出は 1.5b-ii で投入済(SWITCH_THRESHOLD=0.85・別 target 高信頼で新セッション切替・A-6 で 4 intent 四角に自動拡張)
