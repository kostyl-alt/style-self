        ...
        <div className="space-y-3 mb-6">
          <div className="flex gap-2">
            <select
              ...
              onChange={(e) => setCategoryFilter(e.target.value as WardrobeCategory | "all")}
        ...
        {isLoading ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-sm text-gray-400">
              {items.length === 0 ? "まだアイテムがありません" : "該当するアイテムがありません"}
            </p>
            {items.length === 0 && (
              <button
                onClick={() => setShowModal(true)}
            ...
            {statusTab === "considering" ? (
              <div className="space-y-3">
                {filteredItems.map((item) => (
                  <PurchaseCheckPanel key={item.id} item={item} onStatusChange={handleStatusChange} />
                ))}
              </div>
            ) : (
