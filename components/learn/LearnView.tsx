...
interface BrandRow {
  id: string; name: string; name_ja: string | null; country: string;
  description: string; worldview_tags: string[]; era_tags: string[];
  maniac_level: number; price_range: string;
...
interface InspirationRow {
  ...
  category: string; tags: string[]; source_url: string | null; display_order: number;
}

interface InsightCache {
  date: string; insight: LearnInsight; beliefKeywords: string[];
}

const PRICE_LABEL: Record<string, string> = { budget: "¥", mid: "¥¥", high: "¥¥¥", luxury: "¥¥¥¥" };
const THEME_LABEL: Record<string, string> = { material: "素材", silhouette: "シルエット", ratio: "比率・バランス" };
const TYPE_LABEL: Record<string, string> = { insight: "Insight", breakdown: "Breakdown", action: "Action" };
const TYPE_STYLE: Record<string, string> = {
  insight: "bg-blue-50 text-blue-600", breakdown: "bg-amber-50 text-amber-600", action: "bg-emerald-50 text-emerald-600",
};
const CATEGORY_LABEL: Record<string, string> = { silhouette: "シルエット", color: "カラー", material: "素材", detail: "ディテール" };
const COMPATIBILITY_STYLE: Record<string, string> = {
  high: "bg-emerald-50 text-emerald-700 border-emerald-200",
  medium: "bg-amber-50 text-amber-700 border-amber-200",
  low: "bg-red-50 text-red-600 border-red-200",
};
const COMPATIBILITY_LABEL: Record<string, string> = { high: "合いやすい", medium: "部分的に合う", low: "工夫が必要" };
const ADAPTATION_LABEL: Record<string, string> = { main: "メインで取り入れる", accent: "差し色・一点使い", minimal: "ニュアンスだけ参照" };
...
function getMatchingTags(brandTags: string[], userKeywords: string[]): string[] {
  return brandTags.filter((tag) => userKeywords.some((kw) => tag.includes(kw) || kw.includes(tag)));
}

const INSIGHT_CACHE_KEY = "learn_insight_cache";

export default function LearnView({ embedded = false }: { embedded?: boolean }) {
  ...
  useEffect(() => {
    loadInsight();
    fetch("/api/trends").then((r) => r.json()).then((d: { trends?: Trend[] }) => setTrends(d.trends ?? [])).catch(() => {}).finally(() => setTrendsLoading(false));
    fetch("/api/brands/list").then((r) => r.json()).then((d: { brands?: BrandRow[] }) => setBrands(d.brands ?? [])).catch(() => {}).finally(() => setBrandsLoading(false));
    fetch("/api/inspirations").then((r) => r.json()).then((d: { inspirations?: InspirationRow[] }) => setInspirations(d.inspirations ?? [])).catch(() => {}).finally(() => setInspLoading(false));
  ...
  async function handleTranslate(trendId: string) {
    ...
    setTranslating(trendId); setExpandedTrend(trendId);
    try {
  ...
  function loadInsight() {
    try {
      ...
      if (cached) {
        ...
        if (parsed.date === today && isValidInsightCache(parsed)) {
          setInsight(parsed.insight); setBeliefKeywords(parsed.beliefKeywords); setInsightLoading(false);
          return;
        }
    ...
    fetch("/api/ai/learn-insight", { method: "POST" })
      ...
      .then((d: { insight?: LearnInsight; beliefKeywords?: string[]; error?: string }) => {
        if (d.insight) {
          setInsight(d.insight); setBeliefKeywords(d.beliefKeywords ?? []);
          try {
            const cache: InsightCache = { date: new Date().toISOString().split("T")[0], insight: d.insight, beliefKeywords: d.beliefKeywords ?? [] };
  ...
  const Wrapper = embedded
    ? ({ children }: { children: React.ReactNode }) => <div className="space-y-12">{children}</div>
    : ({ children }: { children: React.ReactNode }) => (
        <div className="min-h-screen bg-white pb-24">
          <div className="max-w-lg mx-auto px-4 py-12 space-y-12">{children}</div>
        </div>
      );
              ...
              return (
                <div key={trend.id} className="border border-gray-100 rounded-2xl overflow-hidden">
                  <div className="p-5 space-y-2">
                    <div className="flex items-center gap-2">
                      <span className="text-xs px-2 py-0.5 bg-gray-100 text-gray-500 rounded-full">{CATEGORY_LABEL[trend.category] ?? trend.category}</span>
                    ...
                    <button onClick={() => handleTranslate(trend.id)} disabled={isLoading}
                  ...
                  {isExpanded && (
                    <div className="border-t border-gray-100 px-5 pb-5 pt-4 space-y-4">
                      {isLoading ? (
                        ...
                        <>
                          <div className="flex items-center gap-2">
                            <span className={`text-xs px-2.5 py-1 rounded-full border font-medium ${COMPATIBILITY_STYLE[result.compatibility]}`}>{COMPATIBILITY_LABEL[result.compatibility]}</span>
                          ...
                          <div className="space-y-2">
                            ...
                            {result.specificAdvice.map((advice, i) => (
                              <div key={i} className="flex gap-2"><span className="text-xs text-gray-300 flex-shrink-0 mt-0.5">{i + 1}.</span><p className="text-xs text-gray-700 leading-relaxed">{advice}</p></div>
                            ))}
                          </div>
                          {result.avoidPoints.length > 0 && (
                            <div className="space-y-1.5">
                              ...
                              {result.avoidPoints.map((point, i) => (
                                <p key={i} className="text-xs text-gray-500 leading-relaxed pl-2 border-l-2 border-gray-200">{point}</p>
                              ))}
                            </div>
                          )}
                        </>
          ...
          {INSP_TABS.map((tab) => (
            <button key={tab.value} onClick={() => setActiveCategory(tab.value)}
              className={`flex-shrink-0 px-3 py-1.5 rounded-full text-xs border transition-all ${
                activeCategory === tab.value ? "bg-gray-800 text-white border-gray-800" : "bg-white text-gray-500 border-gray-200 hover:border-gray-400"
              }`}>{tab.label}</button>
          ))}
        </div>
        {inspLoading ? (
            ...
            {filteredInspirations.map((insp) => (
              <div key={insp.id} className="border border-gray-100 rounded-2xl overflow-hidden">
                ...
                <div className="p-5 space-y-3">
                  ...
                  {insp.source_url && (
                    <a href={insp.source_url} target="_blank" rel="noopener noreferrer"
                      className="inline-block text-xs text-gray-400 underline underline-offset-2 hover:text-gray-600">参照元 →</a>
                  )}
                </div>
              </div>
            ))}
          ...
          <div className="relative bg-white w-full sm:max-w-lg sm:rounded-2xl rounded-t-2xl max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 bg-white border-b border-gray-100 px-6 py-4 flex items-center justify-between">
              <div>
                ...
                {selectedBrand.name_ja && <p className="text-xs text-gray-400 mt-0.5">{selectedBrand.name_ja}</p>}
              </div>
            ...
            <div className="px-6 py-6 space-y-6">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {selectedBrand.worldview_tags.map((tag) => <span key={tag} className="px-2 py-0.5 bg-gray-100 text-gray-600 text-xs rounded-full">{tag}</span>)}
                </div>
              </div>
              {selectedBrand.era_tags?.length > 0 && (
                <div>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {selectedBrand.era_tags.map((tag) => <span key={tag} className="px-2 py-0.5 bg-gray-50 text-gray-500 text-xs rounded-full">{tag}</span>)}
                  </div>
                </div>
              )}
              {beliefKeywords.length > 0 && (() => {
                ...
                return matching.length > 0 ? (
                  <div className="bg-emerald-50 rounded-xl p-4">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {matching.map((tag) => <span key={tag} className="px-2 py-0.5 bg-emerald-100 text-emerald-700 text-xs rounded-full">{tag}</span>)}
                    </div>
                  </div>
                ) : null;
              })()}
              {(selectedBrand.official_url || selectedBrand.instagram_url) && (
                <div className="flex gap-3">
                  {selectedBrand.official_url && (
                    <a href={selectedBrand.official_url} target="_blank" rel="noopener noreferrer"
                      className="flex-1 py-2.5 bg-gray-800 text-white rounded-xl text-xs text-center hover:bg-gray-700 transition-colors">公式サイト</a>
                  )}
                  {selectedBrand.instagram_url && (
                    <a href={selectedBrand.instagram_url} target="_blank" rel="noopener noreferrer"
                      className="flex-1 py-2.5 border border-gray-200 text-gray-600 rounded-xl text-xs text-center hover:border-gray-400 transition-colors">Instagram</a>
                  )}
                </div>
              )}
            </div>
